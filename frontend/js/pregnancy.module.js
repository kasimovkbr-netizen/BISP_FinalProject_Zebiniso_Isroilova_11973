...
export function getDayClass(date, periodData) {
  ...
  // Fertile window
  const { fertileStart, fertileEnd } = calculateFertileWindow(
    last,
    cycleLength,
  );
  ...
  if (updateSymptomsBtn) {
    updateSymptomsBtn.addEventListener("click", () => {
      ...
      if (textarea) {
        ...
        setTimeout(() => {
          updateSymptomsBtn.textContent = "Update";
        }, 1500);
      }
    });
...
async function initPeriodCalendar() {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", async () => {
      ...
      const cycleLength = cycleLengthInput
        ? parseInt(cycleLengthInput.value, 10)
        : 28;

      if (!lastPeriodDate) {
        toast("Please enter the last period date.", "warning");
        return;
      }

      if (!validateCycleLength(cycleLength)) {
        toast("Cycle length must be between 21 and 35 days.", "warning");
        return;
      }
