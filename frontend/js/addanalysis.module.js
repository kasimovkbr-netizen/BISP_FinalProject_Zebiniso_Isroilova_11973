    ...
    if (type === "blood") {
      ...
      if (childSelect.value) {
        ...
        if (childData) {
          ...
          if (hgbResult === "low")
            warnings.push(
              "⚠️ Hemoglobin is low: normal for child's age is above " +
                (ageMonths <= 59
                  ? "11.0"
                  : ageMonths <= 143
                    ? "11.5"
          ...
          if (ferResult === "low")
            warnings.push(
              "⚠️ Ferritin is low: normal for child's age is above " +
...
function loadChildrenDropdownRealtime() {
  ...
  unsubChildren = onSnapshot(q, (snapshot) => {
    ...
    if (current) {
      const stillExists = Array.from(childSelect.options).some(
        (o) => o.value === current,
      );
...
function showAnalysisResult(result) {
  ...
  if (costElement) {
    const costText = result.cached
      ? "Free (Cached Result)"
      : `$${(result.cost / 100).toFixed(2)}`;
...
export function showAISummary(result) {
  ...
  if (result && result.relatedCategory) {
    ...
    const title =
      categoryTitles[result.relatedCategory] || result.relatedCategory;
    ...
    link.addEventListener("click", (e) => {
      ...
      if (kbMenuItem) {
        ...
        window.dispatchEvent(
          new CustomEvent("kb-navigate-category", {
...
function showRetryModal(failedResult, analysisData) {
  ...
  modal.originalAnalysisId =
    failedResult.originalAnalysisId || Date.now().toString();
...
async function handleRetryAnalysis(
  ...
  try {
    ...
    if (retryResult.success) {
      ...
      showMessage(
        `Analysis completed successfully on retry! Cost: $${(retryResult.paymentInfo.cost / 100).toFixed(2)}`,
        "success",
      );
