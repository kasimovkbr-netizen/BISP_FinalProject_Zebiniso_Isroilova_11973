...
class FreemiumController {
  ...
  static async checkFreeLimit(userId) {
    try {
      ...
      return {
        ...
        usedAnalyses:
          FREEMIUM_CONFIG.monthlyAnalysisLimit - limitResult.remainingAnalyses,
      };
    } catch (error) {
  ...
  static async consumeFreeAnalysis(userId, analysisType = "default") {
    try {
      ...
      const consumptionResult =
        await FreeUsageTrackingService.consumeFreeAnalysis(userId);
      ...
      if (!consumptionResult.success) {
        return {
          ...
          message:
            consumptionResult.message || "Failed to consume free analysis",
        };
      }
  ...
  static async getUpgradeRecommendations(userId) {
    try {
      ...
      const usageHistory = await FreeUsageTrackingService.getUserHistory(
        userId,
        3,
      );
      const baseRecommendations =
        await FreeUsageTrackingService.getUpgradeRecommendations(
          userId,
          FREEMIUM_CONFIG.monthlyAnalysisLimit,
        );
      ...
      for (const pkg of creditPackages) {
        const costPerAnalysis =
          (pkg.price / (pkg.credits + pkg.bonusCredits)) * 100;
        enhancedRecommendations.push({
          ...
          popular: pkg.popular,
          savings:
            pkg.bonusCredits > 0 ? `${pkg.bonusCredits} bonus credits` : null,
        });
      }
      ...
      for (const tier of subscriptionTiers) {
        const monthlyAnalyses =
          tier.analysisLimit === -1 ? "Unlimited" : tier.analysisLimit;
        const costPerAnalysis =
          tier.analysisLimit > 0
            ? Math.round((tier.monthlyPrice / tier.analysisLimit) * 100)
            : 0;
  ...
  static calculateSubscriptionSavings(tier, usageHistory) {
    ...
    const avgMonthlyUsage =
      usageHistory.reduce((sum, month) => sum + month.analysisCount, 0) /
      usageHistory.length;

    if (tier.analysisLimit === -1) {
      return avgMonthlyUsage > FREEMIUM_CONFIG.monthlyAnalysisLimit
        ? "Unlimited analyses - perfect for heavy usage"
        : null;
    }
      ...
      if (usagePercentage >= FREEMIUM_CONFIG.upgradePrompts.threshold) {
        ...
        if (lastPromptDoc.exists) {
          ...
          const cooldownExpired =
            Date.now() - lastPromptTime.getTime() >
  ...
  static async processUpgradeEffects(userId, upgradeType, upgradeDetails) {
    try {
      ...
      if (upgradeType === "credit") {
        await this.applyCreditUpgradeEffects(
          userId,
          upgradeDetails,
          upgradeResult,
        );
      } else if (upgradeType === "subscription") {
        await this.applySubscriptionUpgradeEffects(
          userId,
          upgradeDetails,
          upgradeResult,
        );
      ...
      console.log(
        `Upgrade effects processed for user ${userId}:`,
        upgradeResult.effectsApplied,
      );
   ...
   */
  static async applyCreditUpgradeEffects(
    userId,
    upgradeDetails,
    upgradeResult,
  ) {
   ...
   */
  static async applySubscriptionUpgradeEffects(
    userId,
    upgradeDetails,
    upgradeResult,
  ) {
  ...
  static async getUserUpgradeStatus(userId) {
    try {
      const upgradeDoc = await db
        .collection("user_upgrade_status")
        .doc(userId)
        .get();
    ...
    try {
      ...
      if (upgradeStatus.upgradeType !== upgradeType) {
        ...
        validation.issues.push(
          `Upgrade type mismatch: expected ${upgradeType}, got ${upgradeStatus.upgradeType}`,
        );
      } else {
