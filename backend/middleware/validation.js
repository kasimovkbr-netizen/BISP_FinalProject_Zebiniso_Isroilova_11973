...
// Allowed analysis types: blood, vitamin (urine removed)
const ALLOWED_ANALYSIS_TYPES = new Set(["blood", "vitamin"]);
...
function validateAnalysisRequest(req, res, next) {
  ...
  if (!values || typeof values !== "object" || Array.isArray(values)) {
    return res.status(400).json({
      success: false,
      error: {
        code: "missing_analysis_values",
        message: "Analysis values must be an object",
      },
    });
  }
...
function validateAnalysisValues(type, values) {
  switch (type) {
    ...
    default:
      return { valid: false, message: `Unsupported analysis type: ${type}` };
  }
}
...
function validateBloodValues(values) {
  ...
  if (typeof ferritin !== "number" || ferritin < 0 || ferritin > 2000) {
    return {
      valid: false,
      message: "Ferritin must be a number between 0 and 2000",
    };
  }
